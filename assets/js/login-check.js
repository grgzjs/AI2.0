window.onload = () => {
	...
	async function auto_redirect() {
		...
		if (!validLogIn && !location.pathname.includes("/login.php")) {
			if (
				!location.pathname.includes("/loginsignup.php") &&
				!location.pathname.includes("/loginforgot.php")
			) {
				window.location.href = "/skysalespro/login.php";
			}
		}

		if (validLogIn &&
			(location.pathname.includes("/login.php") ||
				location.pathname.includes("/loginsignup.php") ||
				location.pathname.includes("/loginforgot.php"))
		) {
			window.location.href = "/skysalespro/dashboard.php";
		}
	}
	...
	async function check_user_validity() {
		...
		for (let i = 0; i < valid_list.length; i++) {
			...
			// 	break;
			// }
            if (location.pathname.includes(valid_location)) {
                valid_user = true;
                break;
            }
		}
		...
		if (!valid_user && !location.pathname.includes("/dashboard.php")) {
			window.location.href = "/skysalespro/dashboard.php";
		}
	}
